...
#from: https://github.com/stranda/holoSimCell/blob/master/R/additional_stats_functions.R
#modified by NE Adams and J Robinson

#Fix major (0) and minor (1) allele designations from simulated data
  maj_min_fix <- function(x){
    if(mean(x, na.rm=TRUE) > 0.5){ # added na.rm for missing data on 6/28/2023
      x <- -1*(x-1)
    }
    x
  }
...
graph_theory <- function(data = data, stats = c("cGD", "betweenness", "closeness"), plot = FALSE) {
	...
	if (TRUE) #changed from FALSE bc textConnection was NOT working for me (7/9/2023) 
	{
